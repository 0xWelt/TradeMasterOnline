...
from tmo.models import AssetType, OrderType, Asset, Order, Trade, TradingPair
...
class TestOrder:
    ...
    def test_order_creation(self):
        """测试订单创建"""
        order = Order(
            id="test-order-1", user_id="user1", order_type=OrderType.BUY, asset=AssetType.BTC, quantity=1.0, price=50000.0
        )
...
class TestTradingPair:
    ...
    def test_trading_pair_creation(self):
        """测试交易对创建"""
        pair = TradingPair(base_asset=AssetType.BTC, quote_asset=AssetType.USDT, current_price=50000.0)
