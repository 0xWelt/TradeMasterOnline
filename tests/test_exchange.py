...
class TestExchange:
    ...
    def test_place_buy_order(self, exchange):
        """测试下买单"""
    ...
    def test_place_sell_order(self, exchange):
        """测试下卖单"""
        order = exchange.place_order(
    ...
    def test_order_matching_same_price(self, exchange):
        """测试相同价格的订单匹配"""
        # 下买单
    ...
    def test_order_matching_different_prices(self, exchange):
        """测试不同价格的订单匹配"""
        # 下高价买单
        buy_order = exchange.place_order(
            user_id="user1", order_type=OrderType.BUY, asset=AssetType.BTC, quantity=1.0, price=50100.0
        )
    ...
    def test_order_book_ordering(self, exchange):
        """测试订单簿排序"""
        # 下多个买单，价格不同
        ...
        buy_order2 = exchange.place_order(
            user_id="user2", order_type=OrderType.BUY, asset=AssetType.BTC, quantity=1.0, price=50100.0
        )

        buy_order3 = exchange.place_order(
            user_id="user3", order_type=OrderType.BUY, asset=AssetType.BTC, quantity=1.0, price=49900.0
        )
        ...
        sell_order1 = exchange.place_order(
            user_id="user4", order_type=OrderType.SELL, asset=AssetType.BTC, quantity=1.0, price=50200.0
        )

        sell_order2 = exchange.place_order(
            user_id="user5", order_type=OrderType.SELL, asset=AssetType.BTC, quantity=1.0, price=50150.0
        )

        sell_order3 = exchange.place_order(
            user_id="user6", order_type=OrderType.SELL, asset=AssetType.BTC, quantity=1.0, price=50300.0
        )
    ...
    def test_cancel_filled_order(self, exchange):
        """测试取消已成交订单"""
        # 下买单和卖单使其成交
        buy_order = exchange.place_order(
            user_id="user1", order_type=OrderType.BUY, asset=AssetType.BTC, quantity=1.0, price=50000.0
        ...
        sell_order = exchange.place_order(
            user_id="user2", order_type=OrderType.SELL, asset=AssetType.BTC, quantity=1.0, price=50000.0
        )
    ...
    def test_get_order_book(self, exchange):
        """测试获取订单簿"""
        # 下一些订单
        exchange.place_order(user_id="user1", order_type=OrderType.BUY, asset=AssetType.BTC, quantity=1.0, price=50000.0)

        exchange.place_order(user_id="user2", order_type=OrderType.SELL, asset=AssetType.BTC, quantity=0.5, price=50100.0)
    ...
    def test_get_trading_pair(self, exchange):
        """测试获取交易对信息"""
    ...
    def test_get_recent_trades(self, exchange):
        """测试获取最近成交记录"""
        # 下订单产生成交
        exchange.place_order(user_id="user1", order_type=OrderType.BUY, asset=AssetType.BTC, quantity=1.0, price=50000.0)

        exchange.place_order(user_id="user2", order_type=OrderType.SELL, asset=AssetType.BTC, quantity=0.5, price=50000.0)
