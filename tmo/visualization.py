...
class ExchangeVisualizer:
    ...
    def record_snapshot(self, exchange: Exchange, step_name: str, description: str = ""):
        ...
        for trade in recent_trades:
            snapshot["recent_trades"].append({"price": trade.price, "quantity": trade.quantity, "timestamp": trade.timestamp})

        self.timeline_data.append(snapshot)
        self.current_step += 1

    def create_visualization(self, exchange: Exchange, output_file: str = "exchange_visualization.html") -> go.Figure:
        """创建交互式可视化图表"""
        # 创建子图
        fig = make_subplots(
            rows=3,
            cols=2,
            subplot_titles=("价格变化时间轴", "订单簿深度", "成交量分布", "订单状态统计", "用户交易活跃度", "时间轴控制"),
            specs=[
        ...
        # 更新布局
        fig.update_layout(
            title={"text": "TradeMasterOnline 交易所可视化", "x": 0.5, "xanchor": "center", "font": {"size": 20}},
            height=1200,
            showlegend=True,
