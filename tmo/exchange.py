...
from .models import (
    AssetType, OrderType, Order, Trade, TradingPair,
    Asset
)
...
class Exchange:
    ...
    def __init__(self):
        ...
        self.trading_pairs: Dict[str, TradingPair] = {
            'BTC/USDT': TradingPair(
                base_asset=AssetType.BTC,
                quote_asset=AssetType.USDT,
    ...
    def place_order(self, user_id: str, order_type: OrderType, 
                   asset: AssetType, quantity: float, price: float) -> Order:
        """下单"""
        # 创建订单
        order = Order(
            id=str(uuid.uuid4()),
    ...
    def get_trading_pair(self, asset: AssetType) -> Optional[TradingPair]:
        """获取交易对信息"""
        pair_symbol = f'{asset.value}/USDT'
